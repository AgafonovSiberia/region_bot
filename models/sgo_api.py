...
async def get_diary(message, start, end, suff):
    ...
    for day in diary['weekDays']:
	    ...
	    for lesson in day['lessons']:
		    result +='<b>'+str(lesson["number"])+'.</b> '+str(lesson["subjectName"])+' - Кабинет: '+str(lesson["room"])+'\n'
    return result
        ...
        for lesson in day["lessons"]:
            try:
                for assignment in lesson["assignments"]: 
                    ...
                    result+='<b>\U000025AA'+str(lesson["subjectName"])+"</b>:"+str(assignment["assignmentName"])+" : <i> "+assign_description+'</i>\n'
            ...
            if 'assignments' in lesson.keys():
                for assignment in lesson['assignments']:
                    if 'mark' in assignment.keys() and 'mark' in assignment['mark']:
                        ...
                        str_json_to_db = {"date":str(date), "lesson":str(lesson['subjectName']), "mark":str(assignment['mark']['mark']), "mark_data":assignment['assignmentName']}
...
async def get_school(message):
    ...
    url_geo = 'https://yandex.ru/maps/?pt='+str(geocode.Decimal(coordinates[0]))+','+str(geocode.Decimal(coordinates[1]))+'&z=158&l=map'
    url_panarams = 'https://yandex.ru/maps/?panorama[point]='+str(geocode.Decimal(coordinates[0]))+','+str(geocode.Decimal(coordinates[1]))+'&panorama[direction]=15,6.060547&panorama[span]=130.000000,71.919192'
    result=(
    '<b>\U0001F538 Наименование: </b>'+info['commonInfo']['fullSchoolName']+'\n\n'
    '<b>\U0001F464 Директор: </b>'+info['managementInfo']['director']+'\n\n'
    '<b>\U0001F4CC Адрес: </b>'+info['contactInfo']['postAddress']+'\n\n'
    '<b>\U0000260E Телефон: </b>'+info['contactInfo']['phones']+'\n\n'
    '<b>\U0001F4E7 Email: </b>'+info['contactInfo']['email']+'\n\n'
